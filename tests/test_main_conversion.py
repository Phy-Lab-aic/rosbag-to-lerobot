...
def test_run_conversion_prefers_trial_episode_metadata_over_root_metadata(
    monkeypatch, tmp_path
):
    main, _ = _import_main(monkeypatch)
    input_root, run_dir = _make_input_tree(tmp_path)
    root_episode_dir = run_dir / "episode"
    trial_episode_dir = run_dir / "trial_1_score95" / "episode"
    root_episode_dir.mkdir(parents=True)
    (root_episode_dir / "metadata.json").write_text("{}")
    captured_task_rows = []
    captured_scoring_calls = []
    ...
    def fake_episode_meta(episode_dir):
        if Path(episode_dir) == root_episode_dir:
            cable_name = "root_cable"
        elif Path(episode_dir) == trial_episode_dir:
            cable_name = "trial_cable"
        else:
            raise AssertionError(f"unexpected episode_dir: {episode_dir}")
        return {
            "cable_type": "cable",
    ...
    monkeypatch.setattr(main, "load_episode_metadata", fake_episode_meta)
    monkeypatch.setattr(main, "build_task_string", lambda episode_meta: "task")
    monkeypatch.setattr(
        main,
        "frames_to_episode",
    ...
    monkeypatch.setattr(
        main,
        "load_scoring_yaml",
        lambda trial_dir, trial_key: captured_scoring_calls.append((trial_dir, trial_key))
        or {"score_total": 1.0},
    )
    monkeypatch.setattr(
        main,
    ...
    monkeypatch.setattr(
        main,
        "write_task_parquet",
        lambda target, rows: captured_task_rows.extend(rows),
    )
    monkeypatch.setattr(main, "write_scoring_parquet", lambda target, rows: None)
    monkeypatch.setattr(main, "write_scene_parquet", lambda target, rows: None)
    monkeypatch.setattr(main, "write_tf_snapshots_parquet", lambda target, rows: None)
    monkeypatch.setattr(main, "write_pose_commands_parquet", lambda target, rows: None)

    result = main.run_conversion("ignored.json")

    assert result == 0
    assert captured_task_rows[0]["cable_name"] == "trial_cable"
    assert captured_task_rows[0]["trial_key"] == "trial_1"
    assert captured_task_rows[0]["trial_score_folder"] == "trial_1_score95"
    assert captured_scoring_calls == [(run_dir / "trial_1_score95", "trial_1")]
...
def test_run_conversion_skips_push_to_hub_when_finalize_metadata_write_fails(
    ...
    monkeypatch.setattr(
        main.logger,
        "warning",
        lambda msg, *args, **kwargs: logged_warnings.append(msg % args if args else msg),
    )
    ...
    assert any(
        "Failed to finalize dataset or write AIC metadata" in message
